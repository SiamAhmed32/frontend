...
export function AuthPageShell({ children, variant = 'login' }: AuthPageShellProps) {
  ...
  const contentHeight = variant === 'register' ? 'min-h-[915px]' : 'min-h-[559px]';
  const contentPadding = variant === 'register' ? 'pt-[59px] pb-[133px]' : 'pt-16 lg:pt-[162px]';
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-white">
      ...
      />
      <section className={`relative mx-auto flex ${contentHeight} w-[min(360px,calc(100vw-32px))] flex-col gap-8 ${contentPadding}`}>
        {children}
      </section>
    </main>
  );
